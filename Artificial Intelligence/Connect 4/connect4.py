...
def minimax(player, board, depth_limit): ####################################################################################3
    """
    ...
    placement = None
    player = board.PLAYER1 # Max player
    # THIS ASSUMES THAT PLAYER1 IS THE MAX PLAYER (AGENT)
    # IF YOU WISH TO PLAY AGAINST THE AGENT, PLEASE PLAY AS PLAYER2
    # THIS WAS STATED IN THE ABOVE HINTS, BUT CONSIDER THIS AS A REMINDER
    
    # IF YOU PLAY AS PLAYER1, WITH AGENT AS PLAYER2... 
    # THE AGENT WILL TRY TO MAXIMIZE YOUR SCORE (AND MINIMIZE ITS OWN) 
    # SO IT WILL BLOCK YOU FROM WINNING ON ACCIDENT, AND NOT TRY TO WIN ITSELF
    
### Please finish the code below ##############################################
###############################################################################
    def value(player, board, depth_limit): # Return utility at terminal states
        if depth_limit == 0 or board.terminal():
            return evaluate(player, board) 
        if player == board.PLAYER1: # Maximize utility 
            return max_value(player, board, depth_limit)
        else:
            return min_value(player, board, depth_limit)

    def max_value(player, board, depth_limit): # Maximize utility
        v = -math.inf
        for c, b in get_child_boards(player, board): 
            c = value(player, b, depth_limit - 1)
            v = max(v, c)
        return v
            
    def min_value(player, board, depth_limit): # Minimize utility
        v = math.inf
        for c, b in get_child_boards(player, board):
            c = value(player, b, depth_limit - 1)
            v = min(v, c)
    ...
    # Placement is the column [0] with the highest value returned by value() with board x[1] using a lambda function
    placement = max(get_child_boards(player, board), key = lambda x: value(player, x[1], depth_limit-1))[0] 
...
###############################################################################
    ...
    def value(player, board, depth_limit, alpha, beta): # Same as minimax with alpha-beta parameters
        if depth_limit == 0 or board.terminal():
            return evaluate(player, board)
    ...
    def max_value(player, board, depth_limit, alpha, beta):
        v = -math.inf
        for c, b in get_child_boards(player, board):
            c = value(player, b, depth_limit - 1, alpha, beta)
            v = max(v, c)
            if v >= beta: # Check v against beta in max nodes
                return v
            alpha = max(alpha, v) # Update alpha in max nodes
        return v
    ...
    def min_value(player, board, depth_limit, alpha, beta):
        ...
        for c, b in get_child_boards(player, board):
            c = value(player, b, depth_limit - 1, alpha, beta)
            v = min(v, c)
            if v <= alpha: # Check v against alpha in min nodes
                return v
            beta = min(beta, v) # Update beta in min nodes
        return v
    
    # Placement function is the same as minimax with added alpha-beta parameters
    placement = max(get_child_boards(player, board), key = lambda x: value(player, x[1], depth_limit-1, alpha, beta))[0] 
...
def expectimax(player, board, depth_limit):
    ...
    placement = None
    # No assigned max player, as it is assumed to be the initial player

### Please finish the code below ##############################################
###############################################################################
    def value(player, board, depth_limit): # value and max_value are the same as minimax
        if depth_limit == 0 or board.terminal():
            return evaluate(player, board)
        if player == board.PLAYER1:
            return max_value(player, board, depth_limit)
    ...
    def exp_value(player, board, depth_limit):
        ...
        for c, b in get_child_boards(player, board):
            c = value(player, b, depth_limit - 1)
            v = v + (1/7) * c # 7 possible placements in the board (Generally Speaking, but this is a simpler implementation)
        return v
    
    # Placement is the column [0] with the highest value returned by value() with board x[1]
    placement = max(get_child_boards(player, board), key = lambda x: value(player, x[1], depth_limit-1))[0] 
