...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    dtype = X.dtype # get the dtype of the input
    
    # compute hidden layer representation (after the ReLU) on GPU
...
def nn_get_search_params():
    ...
    regularization_strengths = [1e-1, 0, 1e0]
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    ...
                    # Train the neural network instance
                    stats = net.train(
                        data_dict["X_train"],
                        data_dict["y_train"],
                        data_dict["X_val"],
                    ...
                    # Compute the validation accuracy of the trained neural network
                    val_acc = (net.predict(data_dict["X_val"]) == data_dict["y_val"]).to(torch.float32).mean()
                    
                    # If the current neural network has a better validation accuracy, update the best neural network instance
